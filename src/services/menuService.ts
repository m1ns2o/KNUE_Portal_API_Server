...
export class MenuService {
	...
	private async fetchMenuHtml(): Promise<string> {
		try {
			...
			throw new Error(
				"KNUE 웹사이트에서 메뉴 데이터를 가져오는 데 실패했습니다"
			);
		}
	}
	...
	public async fetchAndStoreMenuData(): Promise<MenuData> {
		try {
			...
			console.error("fetchAndStoreMenuData 오류:", error);
			throw error;
		}
	}
	...
	/**
	 * 특정 식당의 메뉴 데이터를 가져옴
	 * @param cafeteriaType 식당 타입 (교직원식당 또는 기숙사식당)
	...
	public async getDayMenu(day: string): Promise<any> {
		...
		return {
			교직원식당: menuData.교직원식당?.[day] || null,
			기숙사식당: menuData.기숙사식당?.[day] || null,
		};
	}
